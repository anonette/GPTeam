...
class Agent(BaseModel):
    # [Previous class attributes remain the same...]

    @property
    async def allowed_locations(self) -> list[Location]:
        """Get the single location that this agent is allowed to be in.
        Returns a list with exactly one location for compatibility with existing code.
        Raises ValueError if agent is not assigned to exactly one location."""
        ...
        if len(data) == 0:
            raise ValueError(f"Agent {self.full_name} (ID: {self.id}) is not assigned to any location")
        if len(data) > 1:
